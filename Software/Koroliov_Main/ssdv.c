...
static char ssdv_have_marker_data(ssdv_t *s)
{
  ...
  case J_SOF0:
    ...
    if(d[0] != 8) return(SSDV_ERROR-4);

    /* The image must have 3 components (Y'Cb'Cr) */
    if(d[5] != 3) return(SSDV_ERROR-5);

    /* Maximum image is 4080x4080 */
    if(s->width > 4080 || s->height > 4080) return(SSDV_ERROR-6);

    /* The image dimensions must be a multiple of 16 */
    if((s->width & 0x0F) || (s->height & 0x0F)) return(SSDV_ERROR-7);
    ...
    {
      uint8_t *dq = &d[i * 3 + 6];
      if(dq[0] != i + 1) return(SSDV_ERROR-8);
      ...
      else if(dq[0] != 1 && dq[1] != 0x11) return(SSDV_ERROR-10);
    }
    ...
    if(l > 0xFFFF) return(SSDV_ERROR-11);
  ...
  case J_SOS:
    /* The image must have 3 components (Y'Cb'Cr) */
    if(d[0] != 3) return(SSDV_ERROR-12);

    for(i = 0; i < 3; i++)
    {
      uint8_t *dh = &d[i * 2 + 1];
      if(dh[0] != i + 1) return(SSDV_ERROR-13);
    }
    ...
    if(!s->sdqt[0] || !s->sdqt[1]) return(SSDV_ERROR-14);

    if(!s->sdht[0][0] || !s->sdht[0][1] ||
      !s->sdht[1][0] || !s->sdht[1][1]) return(SSDV_ERROR-15);
...
char ssdv_enc_get_packet(ssdv_t *s)
{
  ...
  while(s->in_len)
  {
    ...
    case S_INT:
      ...
      else if(r != SSDV_FEED_ME) return(SSDV_ERROR-16);
      break;
